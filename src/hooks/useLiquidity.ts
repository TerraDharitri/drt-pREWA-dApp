...
const SLIPPAGE_BPS = 50n; // 0.50% - FIX: Re-added the missing constant
...
export const useLiquidity = (opts: UseLiquidityOpts = {}) => {
  ...
  // FIX: Changed back to the correct useReadContract hook
  ...
  const addLiquidity = async (tokenA: Address, tokenB: Address, amountA: string, amountB: string) => { if (!address || !lmAddress) { toast.error("Connect your wallet."); return; } const n = fnInputs(lmAbi, "addLiquidity"); const aDesired = parseUnits(amountA, decimalsOf(tokenA)); const bDesired = parseUnits(amountB, decimalsOf(tokenB)); const aMin = aDesired - (aDesired * SLIPPAGE_BPS) / 10000n; const bMin = bDesired - (bDesired * SLIPPAGE_BPS) / 10000n; if (prewaAddress) { const aIsPrewa = tokenA.toLowerCase() === prewaAddress.toLowerCase(); lastOtherTokenRef.current = (aIsPrewa ? tokenB : tokenA) as Address; } if (n === 8) { await askWallet({ address: lmAddress, abi: lmAbi, functionName: "addLiquidity", args: [tokenA, tokenB, aDesired, bDesired, aMin, bMin, address, deadline()] }, "Requesting signature to add liquidity…", "add"); return; } if (n === 6) { if (!prewaAddress) { toast.error("pREWA address missing for 6-arg addLiquidity."); return; } const aIsPrewa = tokenA.toLowerCase() === prewaAddress.toLowerCase(); const otherToken = (aIsPrewa ? tokenB : tokenA) as Address; const prewaAmount = aIsPrewa ? aDesired : bDesired; const otherAmount = aIsPrewa ? bDesired : aDesired; const prewaMin = aIsPrewa ? aMin : bMin; const otherMin = aIsPrewa ? bMin : aMin; lastOtherTokenRef.current = otherToken; await askWallet({ address: lmAddress, abi: lmAbi, functionName: "addLiquidity", args: [otherToken, prewaAmount, otherAmount, prewaMin, otherMin, deadline()] }, "Requesting signature to add liquidity…", "add"); return; } toast.error(`Unsupported addLiquidity ABI (inputs=${n}).`); };
  const addLiquidityBNB = async (otherToken: Address, amountToken: string, amountBNB: string) => { if (!address || !lmAddress) { toast.error("Connect your wallet."); return; } lastOtherTokenRef.current = otherToken; const tokenDesired = parseUnits(amountToken, decimalsOf(otherToken)); const tokenMin = tokenDesired - (tokenDesired * SLIPPAGE_BPS) / 10000n; const bnbDesired = parseUnits(amountBNB, 18); const bnbMin = bnbDesired - (bnbDesired * SLIPPAGE_BPS) / 10000n; const n = fnInputs(lmAbi, "addLiquidityBNB"); if (n === 6) { await askWallet({ address: lmAddress, abi: lmAbi, functionName: "addLiquidityBNB", args: [otherToken, tokenDesired, tokenMin, bnbMin, address, deadline()], value: bnbDesired }, "Requesting signature to add liquidity…", "add"); return; } if (n === 5) { await askWallet({ address: lmAddress, abi: lmAbi, functionName: "addLiquidityBNB", args: [otherToken, tokenDesired, tokenMin, bnbMin, deadline()], value: bnbDesired }, "Requesting signature to add liquidity…", "add"); return; } toast.error(`Unsupported addLiquidityBNB ABI (inputs=${n}).`); };
