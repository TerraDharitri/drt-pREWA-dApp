...
export function RemoveLiquidityForm() {
  ...
  const hasSufficientBalance = useMemo(() => {
    ...
    return parseUnits(lpAmount, lpBalance.decimals) <= lpBalance.value;
  }, [lpAmount, lpBalance, isAmountValid]);

  const needsApproval = isAmountValid && hasSufficientBalance && selectedLp && lpBalance && lpBalance.value > 0n && (!allowance || allowance < parseUnits(lpAmount, 18));
  ...
  const handleRemove = () => {
    ...
    const executeRemove = () => {
      ...
      const isBNBPair = tokens.find(t => t.symbol === 'BNB' && isAddressEqual(t.address, selectedLp.otherTokenAddress));
      if (isBNBPair) {
  ...
  return (
    <Card className="max-w-md mx-auto">
      ...
      <CardContent className="space-y-4">
        ...
        <Button onClick={handleRemove} className="w-full" disabled={isLoading || !isAmountValid || !selectedLp || !hasSufficientBalance}>
